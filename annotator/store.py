...
class User:
    def __init__(self, id, consumer_key):
        self.id = id
        self.consumer = Consumer(consumer_key)
...
def before_request():
    ...
    user = User('default_user', 'default_key')
    if request.is_json:
        req_data = request.get_json()
        if req_data:
            user.id = req_data.get('user')
            user.consumer.key = 'mockconsumer'
